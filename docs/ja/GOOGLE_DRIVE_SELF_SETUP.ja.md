...
4. `実 Google Drive API` では、ビューア対応形式（テキスト/Markdown/画像）の本文閲覧が可能です。
5. Google Drive への書き戻しは手動で実行できます（コンテキストメニュー `Google Driveへ書き戻し`）。
6. 閲覧時に取得した Google Drive ファイルは一時フォルダへキャッシュされ、サインアウト時に削除されます。
7. Google Drive ファイルを外部アプリで開く場合は、ローカル作業コピーを開きます（自動書き戻しは行いません）。
8. 書き戻しは手動です。同じ Google Drive ファイルを選択し、コンテキストメニューの `Google Driveへ書き戻し` を実行します。
9. 書き戻しには OAuth スコープ `https://www.googleapis.com/auth/drive` が必要です。
...
9. サインアウト時には refresh token を削除し、Google Drive 閲覧用の一時キャッシュも削除されます。
...
5. 認証成功後も Drive の実ファイルが見えない
   - `バックエンド種別` が `スタブ（テスト用仮想データ）` になっている場合は、実データ表示はできません（スタブビルドです）。
6. 「ローカル作業コピーがありません」と表示される
   - 先に対象ファイルを外部アプリで 1 回開いてください（作業コピー作成が必要です）。
7. 書き戻しで競合エラーになる
   - 原因: ローカル作業コピーを作成した後に、Google Drive 側ファイルが更新されています。
   - 現在の挙動: 競合時は書き戻しを中止し、base revision は自動更新しません（最新リモートへ自動追従しません）。
   - 競合時は、ReflexFiles が自動で設定画面を開き、競合対処ガイドを表示します。
   - 安全な復旧手順:
     1. `Google Driveへ書き戻し` を連打しない。
     2. ReflexFiles で同じ Google Drive ファイルを外部アプリで開き直す（最新リモート基準の作業コピーを作成/更新）。
     3. エディタや差分ツールで、未反映のローカル変更を最新作業コピーへ手動マージする。
     4. マージ結果をローカル保存する。
     5. `Google Driveへ書き戻し` を再実行する。
     6. 成功ステータス（`ローカル作業コピーを Google Drive に書き戻しました: ...`）を確認する。
   - 競合が続く場合は、他ユーザー/他プロセスが更新中です。再度最新状態からマージしてください。
8. 書き戻し時に `Request had insufficient authentication scopes` が出る
   - 読み取り専用スコープで認証されたトークンです。`サインアウト` 後に、`サインイン開始` → `サインイン完了` をやり直し、`https://www.googleapis.com/auth/drive` を取り直してください。
