...
### デュアルペインモード（F3 で切替）
1. **メニューバー**
2. **左ペイン** — PATH バー + ファイル一覧
3. **右ペイン** — PATH バー + ファイル一覧
4. **ステータスバー**

デュアルペインモードではツリービューは非表示になります。各ペインは独立して操作できます。
...
- 補完候補表示中に **Enter** を押すと、現在の候補を確定してそのディレクトリへ移動します（候補表示は終了し、フォーカスは一覧へ戻ります）
- 補完候補表示中に **\\ / ¥** を入力すると、現在候補のディレクトリを確定して続けて入力できます
- **Tab / Shift+Tab** でフォーカス移動（ファイル一覧 → PATH → ツリー → 一覧）
  - デュアルペインモードでは **Ctrl+Tab** でアクティブペインを切替えます
...
- **デュアルペインレイアウト**
  - **F3** でシングル/デュアルペインを切替え
  - **Ctrl+Tab** でアクティブペインを切替え
  - 各ペインが独立したパス・選択状態で操作可能
  - デュアルペインモード中はツリービューは非表示
- **検索**
...
- **ZIP**
  ...
  - ZIP 解凍
- **ドラッグ&ドロップ（ローカル限定）**
  - エクスプローラー → ReflexFiles（ローカルフォルダ表示中にドロップ）
  - ReflexFiles → エクスプローラー（ファイルリスト行で `Ctrl+Alt + 左クリック` 後、そのままドラッグ、コピーのみ）
  - Google Drive パス（`gdrive://`）は対象外
- **ソート**
  - ソートメニューから選択
- **外部アプリ起動**
...
| レイアウト | デュアル/シングルペイン切替 | F3 |
| レイアウト | アクティブペイン切替 | Ctrl+Tab |
...
### 例4: 2つのフォルダ間でファイルをコピーする（デュアルペイン）
1. **F3** でデュアルペインモードに切替え
2. 左右各ペインをコピー元・コピー先フォルダに移動
3. コピー元ペインでファイルを選択し **Ctrl+C**
4. **Ctrl+Tab** でコピー先ペインに切替え、**Ctrl+V** でペースト
...
- ドラッグ&ドロップはローカルパスのみ対応です。
  - `gdrive://` パスは対象外です。
  - エクスプローラーへの書き出しは、ファイルリスト行で `Ctrl+Alt + 左クリック` して開始します（コピーのみ）。
